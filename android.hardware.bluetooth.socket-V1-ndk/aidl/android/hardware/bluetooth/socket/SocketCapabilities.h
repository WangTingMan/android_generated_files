...
#include <aidl/android/hardware/bluetooth/socket/RfcommCapabilities.h>
...
public:
  ...
  ::aidl::android::hardware::bluetooth::socket::LeCocCapabilities leCocCapabilities;
  ::aidl::android::hardware::bluetooth::socket::RfcommCapabilities rfcommCapabilities;

  binder_status_t readFromParcel(const AParcel* parcel);
  binder_status_t writeToParcel(AParcel* parcel) const;

  inline bool operator==(const SocketCapabilities& _rhs) const {
    return std::tie(leCocCapabilities, rfcommCapabilities) == std::tie(_rhs.leCocCapabilities, _rhs.rfcommCapabilities);
  }
  inline bool operator<(const SocketCapabilities& _rhs) const {
    return std::tie(leCocCapabilities, rfcommCapabilities) < std::tie(_rhs.leCocCapabilities, _rhs.rfcommCapabilities);
  }
  ...
  inline std::string toString() const {
    ...
    _aidl_os << "SocketCapabilities{";
    _aidl_os << "leCocCapabilities: " << ::android::internal::ToString(leCocCapabilities);
    _aidl_os << ", rfcommCapabilities: " << ::android::internal::ToString(rfcommCapabilities);
    _aidl_os << "}";
    return _aidl_os.str();
  }
};
