#define LOG_TAG "android.audio.policy.configuration"
    ...
    namespace audio {
        namespace policy {
            namespace configuration {
                ...
                MixPorts::MixPort::MixPort(std::vector<Profile> profile, std::vector<Gains> gains, std::string name, Role role, std::optional<std::vector<AudioInOutFlag>> flags, std::optional<unsigned int> maxOpenCount, std::optional<unsigned int> maxActiveCount, std::optional<unsigned int> recommendedMuteDurationMs) : profile_(std::move(profile)), gains_(std::move(gains)), name_(std::move(name)), role_(role), flags_(std::move(flags)), maxOpenCount_(maxOpenCount), maxActiveCount_(maxActiveCount), recommendedMuteDurationMs_(recommendedMuteDurationMs) {
                }
                ...
                const unsigned int& MixPorts::MixPort::getRecommendedMuteDurationMs() const {
                    _xsdc_assert(hasRecommendedMuteDurationMs());
                    return recommendedMuteDurationMs_.value();
                }

                bool MixPorts::MixPort::hasRecommendedMuteDurationMs() const {
                    return recommendedMuteDurationMs_.has_value();
                }
                ...
                MixPorts::MixPort MixPorts::MixPort::read(xmlNode *root) {
                    ...
                    _raw = getXmlAttribute(root, "recommendedMuteDurationMs");
                    std::optional<unsigned int> recommendedMuteDurationMs = std::nullopt;
                    if (_raw != "") {
                        unsigned int _value = static_cast<unsigned int>(stoul(_raw));
                        recommendedMuteDurationMs = _value;
                    }
                    ...
                    MixPorts::MixPort instance(profile, gains, name, role, flags, maxOpenCount, maxActiveCount, recommendedMuteDurationMs);
                    return instance;
                }
                ...
                Profile::Profile(std::optional<std::string> format, std::optional<std::vector<int64_t>> samplingRates, std::optional<std::vector<AudioChannelMask>> channelMasks) : format_(std::move(format)), samplingRates_(std::move(samplingRates)), channelMasks_(std::move(channelMasks)) {
                }
                ...
                Profile Profile::read(xmlNode *root) {
                    ...
                    Profile instance(format, samplingRates, channelMasks);
                    return instance;
                }

                Gains::Gain::Gain(std::vector<AudioGainMode> mode, std::optional<AudioChannelMask> channel_mask, std::optional<int> minValueMB, std::optional<int> maxValueMB, std::optional<int> defaultValueMB, std::optional<int> stepValueMB, std::optional<int> minRampMs, std::optional<int> maxRampMs, std::optional<bool> useForVolume) : mode_(std::move(mode)), channel_mask_(channel_mask), minValueMB_(minValueMB), maxValueMB_(maxValueMB), defaultValueMB_(defaultValueMB), stepValueMB_(stepValueMB), minRampMs_(minRampMs), maxRampMs_(maxRampMs), useForVolume_(useForVolume) {
                }
                ...
                Gains::Gain Gains::Gain::read(xmlNode *root) {
                    ...
                    Gains::Gain instance(mode, channel_mask, minValueMB, maxValueMB, defaultValueMB, stepValueMB, minRampMs, maxRampMs, useForVolume);
                    return instance;
                }
