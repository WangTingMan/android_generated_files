...
namespace bluetooth {
namespace lmp_event {
class ANDROIDHARDWAREBLUETOOTHLMPEVENTV1NDK_API BnBluetoothLmpEventCallback : public ::ndk::BnCInterface<IBluetoothLmpEventCallback> {
public:
  BnBluetoothLmpEventCallback();
  virtual ~BnBluetoothLmpEventCallback();
...
class IBluetoothLmpEventCallbackDelegator : public BnBluetoothLmpEventCallback {
public:
  explicit IBluetoothLmpEventCallbackDelegator(const std::shared_ptr<IBluetoothLmpEventCallback> &impl) : _impl(impl) {
     int32_t _impl_ver = 0;
     if (!impl->getInterfaceVersion(&_impl_ver).isOk()) {;
        __assert2(__FILE__, __LINE__, __PRETTY_FUNCTION__, "Delegator failed to get version of the implementation.");
     }
     if (_impl_ver != IBluetoothLmpEventCallback::version) {
  ...
  ::ndk::ScopedAStatus onEventGenerated(const ::aidl::android::hardware::bluetooth::lmp_event::Timestamp& in_timestamp, ::aidl::android::hardware::bluetooth::lmp_event::AddressType in_addressType, const std::array<uint8_t, 6>& in_address, ::aidl::android::hardware::bluetooth::lmp_event::Direction in_direction, ::aidl::android::hardware::bluetooth::lmp_event::LmpEventId in_lmpEventId, char16_t in_connEventCounter) override {
    return _impl->onEventGenerated(in_timestamp, in_addressType, in_address, in_direction, in_lmpEventId, in_connEventCounter);
  }
  ::ndk::ScopedAStatus onRegistered(bool in_status) override {
    return _impl->onRegistered(in_status);
  }
protected:
private:
  std::shared_ptr<IBluetoothLmpEventCallback> _impl;
};
