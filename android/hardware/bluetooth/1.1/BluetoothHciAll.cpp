...
::android::hardware::Return<void> IBluetoothHci::getHashChain(getHashChain_cb _hidl_cb){
    ...
    uint8_t v1Detailes[] = { 54,47,209,194,22,65,194,34,79,59,128,195,13,151,151,185,136,250,63,52,66,67,213,49,186,115,197,83,119,154,87,99 };
    for( auto i = 0; i < 32; ++i )
    {
        v1[i] = v1Detailes[i];
    }

    uint8_t v2Detailes[] = { 52,124,231,70,129,86,7,86,127,95,59,83,228,128,9,152,202,90,185,53,81,65,240,136,15,192,207,12,31,197,195,85 };
    for( auto i = 0; i < 32; ++i )
    {
        v2[i] = v2Detailes[i];
    }
