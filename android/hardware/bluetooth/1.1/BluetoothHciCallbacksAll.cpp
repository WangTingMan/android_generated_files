...
::android::hardware::Return<void> IBluetoothHciCallbacks::getHashChain(getHashChain_cb _hidl_cb){
    ...
    uint8_t v1Detailes[] = { 64,171,44,104,102,193,141,50,186,246,228,158,48,83,148,158,121,96,31,86,150,58,121,30,147,230,139,158,225,143,113,141 };
    for( auto i = 0; i < 32; ++i )
    {
        v1[i] = v1Detailes[i];
    }

    uint8_t v2Detailes[] = { 131,95,65,190,34,129,191,178,47,62,51,198,250,135,11,222,123,194,30,55,229,207,186,249,163,111,255,23,6,50,247,84 };
    for( auto i = 0; i < 32; ++i )
    {
        v2[i] = v2Detailes[i];
    }
