...
::android::hardware::Return<void> IBluetoothAudioProvidersFactory::getHashChain(getHashChain_cb _hidl_cb){
    ...
    uint8_t v1Detailes[] = { 212,23,169,33,44,143,150,227,160,106,47,34,28,140,87,86,199,101,53,91,43,129,222,43,42,101,212,201,238,232,84,1 };
    for (auto i = 0; i < 32; ++i)
    {
        v1[i] = v1Detailes[i];
    }

    uint8_t v2Detailes[] = { 247,67,31,62,62,78,51,135,252,111,39,166,207,66,62,221,205,130,74,57,93,196,52,157,48,44,153,90,180,74,152,149 };
    for (auto i = 0; i < 32; ++i)
    {
        v2[i] = v2Detailes[i];
    }
